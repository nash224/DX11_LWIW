...
#pragma endregion
...

	{
		...
		BodyRenderer->SetFrameEvent("Ellie_Basic_Cheer", 10, [&](GameEngineSpriteRenderer* _Renderer)
			{
				std::shared_ptr<CheerFx> Fx = GetLevel()->CreateActor<CheerFx>(EUPDATEORDER::Objects);
...

	{
		...
		BodyRenderer->SetFrameEvent("Ellie_Basic_Fail", 10, [&](GameEngineSpriteRenderer* _Renderer)
			{
				const std::shared_ptr<FailFx> FX = GetLevel()->CreateActor<FailFx>(EUPDATEORDER::Objects);
				FX->Transform.SetLocalPosition(Transform.GetLocalPosition());

				std::vector<float> FailFxInter;
				std::vector<float> FailInter = BodyRenderer->FindAnimation("Ellie_Basic_Fail")->Inter;
				for (int i = 5; i < FailInter.size(); i++)
				{
					FailFxInter.push_back(FailInter[i]);
				}

				FX->SetAnimationInter(FailFxInter);
